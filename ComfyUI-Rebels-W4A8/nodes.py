...
def dequantize_w4a8(qdata, s_rel, s_channel, codebook, cfg, device, dtype):
    """Reproduces the converter's dequant exactly (grid-rounded int8 path)."""
    n, k = cfg["orig_shape"]
    bits = cfg["bits"]
    gsz = cfg["group_size"]
    cr = cfg["convrot_groupsize"]
    q = _unpack(qdata.to(device), k, bits)
    groups = k // gsz
    s_rel = s_rel.to(device).float()
    cb = codebook.to(device).float()
    grid = (cb.view(1, 1, -1) * s_rel.unsqueeze(-1)).round().clamp(-127, 127)
    picked = torch.gather(grid, 2, q.view(n, groups, gsz).long())
    rot = (picked * s_channel.to(device).float().view(n, 1, 1)).view(n, k)
    h = _hadamard(cr, device, torch.float32)
    w = (rot.view(n, k // cr, cr) @ h.T).view(n, k)
    return w.to(dtype)
...
class W4A8Ops(comfy.ops.manual_cast):
    class Linear(comfy.ops.manual_cast.Linear):
        """Stores packed weights outside the Parameter system (GGUF-style)."""
        ...
        def _save_to_state_dict(self, destination, prefix, keep_vars):
            if self.w4a8 is not None:
                destination[prefix + "weight"] = self.w4a8
        ...
        def weight(self, value):
            if isinstance(value, W4A8Tensor):
                self.w4a8 = value
                self._parameters.pop("weight", None)
            elif value is None:
        ...
        def forward(self, x, *args, **kwargs):
            if self.w4a8 is not None:
                weight = _dequant_any(self.w4a8, x.device, x.dtype)
                bias = None
                if self.bias is not None:
