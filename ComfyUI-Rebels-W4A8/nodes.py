...
def _load_gguf_module(name):
    """Import a submodule of the installed ComfyUI-GGUF pack.

    Their modules use relative imports (from .dequant import ...), so the file
    cannot be exec'd standalone -- it needs a parent package. We register a
    namespace package pointing at their folder, WITHOUT executing their
    __init__.py (that would re-register their nodes).
    """
    import importlib
    ...
    pkgname = "_rebels_gguf_pkg"
    if pkgname not in sys.modules:
        here = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        target = None
        for folder in os.listdir(here):
            norm = folder.lower().replace("_", "-")
            if norm in ("comfyui-gguf", "comfyui-gguf-main"):
                cand = os.path.join(here, folder)
                if os.path.exists(os.path.join(cand, "loader.py")):
                    target = cand
                    break
        if target is None:
            raise RuntimeError(
                "ComfyUI-GGUF not found in custom_nodes -- this node loads "
                "GGUF through that pack and cannot work without it.")
        spec = importlib.machinery.ModuleSpec(pkgname, None, is_package=True)
        pkg = importlib.util.module_from_spec(spec)
        pkg.__path__ = [target]
        sys.modules[pkgname] = pkg
    return importlib.import_module(f"{pkgname}.{name}")
...
class RebelsGGUFUnetLoaderMeta:
    ...
    def load(self, unet_name, model_type="animate2"):
        ...
        if not path:
            raise RuntimeError(
                f"could not resolve '{unet_name}' in {self.FOLDERS}; put the "
                f".gguf in ComfyUI/models/diffusion_models")

        try:
            result = loader.gguf_sd_loader(path)
        except Exception:
            logging.error("[Rebels GGUF] gguf_sd_loader failed:\n%s",
                          traceback.format_exc())
            raise

        # current ComfyUI-GGUF returns (sd, extra); older versions return sd
        if isinstance(result, tuple):
            sd, extra = result[0], (result[1] if len(result) > 1 else {})
        else:
            sd, extra = result, {}

        # merge into whatever metadata the file already carries
        ...
        if model_type:
            ...
            if metadata.get("config"):
                try:
                    ...
                    cfg = {}
            cfg.setdefault("transformer", {})["model_type"] = model_type
            metadata["config"] = json.dumps(cfg)
        if not metadata:
            metadata = None

        # ComfyUI-GGUF passes an INSTANCE of GGMLOps; older builds took the
        # class. Try instance first, fall back to the class.
        ops_candidates = []
        try:
            ops_candidates.append(ops_mod.GGMLOps())
        except Exception:
            pass
        ops_candidates.append(ops_mod.GGMLOps)

        fn = comfy.sd.load_diffusion_model_state_dict
        accepts_metadata = "metadata" in inspect.signature(fn).parameters
        if metadata is not None and not accepts_metadata:
            raise RuntimeError(
                "this ComfyUI build's load_diffusion_model_state_dict has no "
                "metadata parameter, so model_type cannot be supplied; "
                "update ComfyUI or leave model_type blank")

        model, errors = None, []
        for ops in ops_candidates:
            try:
                kwargs = {"model_options": {"custom_operations": ops}}
                ...
                errors.append(f"{type(ops).__name__}: {e}\n"
                              f"{traceback.format_exc()}")
        if model is None:
            for e in errors:
                logging.error("[Rebels GGUF] load attempt failed:\n%s", e)
            raise RuntimeError(
                "could not load this GGUF. If the traceback above mentions "
                "startswith or a missing key, the block names may still be "
                "nested (blocks.N.block.*) -- flatten with "
                "gguf_rename_keys.py --strip \".block.\". Full details are "
                "in the console.")

        try:
            logging.info(
                "[Rebels GGUF] loaded %s | model_type=%s | model class=%s",
                unet_name, model_type or "(none)", type(model.model).__name__)
        except Exception:
            pass
        return (model,)
