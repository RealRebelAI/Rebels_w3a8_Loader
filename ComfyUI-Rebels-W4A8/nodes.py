...
class W4A8Tensor(torch.Tensor):
    ...
    def numel(self):
        # report TRUE packed byte count (qdata + scales + codebook) so
        # ComfyUI's load AND unload accounting both match reality
        packed = torch.Tensor._make_subclass(torch.Tensor, self)
        total = packed.nelement()
...
class RebelsW4A8UnetLoader:
    ...
    def INPUT_TYPES(cls):
        return {"required": {
            ...
            "workspace_multiplier": ("FLOAT", {"default": 2.0, "min": 1.0,
                                               "max": 8.0, "step": 0.25,
    ...
    def load(self, unet_name, workspace_multiplier=2.0):
